...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (hasPermission(Permission.VIEW_TIPS)) {
      const unsubTips = onSnapshot(query(dbCollections.submissions, orderBy("timestamp", "desc"), limit(10)), (snap) => {
        const tips = snap.docs.map(d => ({ id: d.id, ...d.data() } as CitizenSubmission));
        const activeNewTips = tips.filter(t => t.status === 'Neu');
        
        // Show toast only if more tips than before and notification was not already shown
  ...
  useEffect(() => {
    ...
    // Listen for reminders across all reports
  ...
  const updateTipStatus = async (tipId: string, newStatus: CitizenSubmission['status']) => {
    try {
      const tipRef = doc(db, "submissions", tipId);
      await updateDoc(tipRef, { status: newStatus });
    } catch (e) {
      console.error("Fehler beim Aktualisieren des Hinweises:", e);
    }
  };
  ...
  return (
    <div className="h-full w-full overflow-hidden flex flex-col relative font-sans select-none">
          ...
          <div className="bg-[#0f172a]/95 backdrop-blur-2xl border border-blue-500/30 rounded-2xl p-5 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-center gap-5 max-w-sm">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs font-bold text-white uppercase tracking-tight">
                {newTips.length} neue Bürgerhinweise
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <button onClick={() => navigate('/tips')} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all">Ansehen</button>
              <button onClick={() => setShowNotification(false)} className="text-slate-500 hover:text-white text-[9px] font-black uppercase tracking-widest text-center">Schließen</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 p-10 lg:p-14 flex flex-col xl:flex-row gap-12">
        ...
        <div className="w-full xl:w-96 flex flex-col gap-8 animate-in fade-in slide-in-from-right-4 duration-1000 delay-300">
           ...
           <div className="bg-[#0f172a]/60 backdrop-blur-xl border border-white/5 rounded-[40px] p-8 shadow-2xl">
              ...
              <div className="space-y-4">
                {upcomingReminders.map(({caseId, caseTitle, reminder}) => (
                  <div key={reminder.id} className="group bg-white/5 border border-white/5 hover:border-blue-500/30 p-5 rounded-2xl transition-all">
                    <div className="flex justify-between items-start mb-2">
                       ...
                       <span className={`text-[8px] font-mono px-2 py-0.5 rounded border ${new Date(reminder.dueDate) < new Date() ? 'bg-red-500/20 border-red-500/40 text-red-500' : 'bg-blue-500/10 border-blue-500/30 text-blue-400'}`}>
                ...
                {upcomingReminders.length === 0 && <div className="py-8 text-center opacity-20 text-[10px] font-black uppercase tracking-widest">Keine Fristen</div>}
              </div>
           </div>

           {/* NEUE Bürgerhinweise Widget */}
           {hasPermission(Permission.VIEW_TIPS) && (
             <div className="bg-[#0f172a]/60 backdrop-blur-xl border border-white/5 rounded-[40px] p-8 shadow-2xl">
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/5">
                  <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-amber-500">Neue Bürgerhinweise</h3>
                  <span className="text-[10px] bg-amber-600/20 text-amber-400 px-3 py-1 rounded-full border border-amber-500/20 font-black">{newTips.length}</span>
                </div>
                <div className="space-y-4">
                  {newTips.slice(0, 4).map(tip => (
                    <div key={tip.id} className="bg-amber-600/5 border border-amber-500/10 hover:border-amber-500/30 p-5 rounded-2xl transition-all">
                      <div className="flex justify-between items-start mb-2">
                         <span className="text-[8px] font-black uppercase tracking-widest text-amber-500/60 truncate max-w-[150px]">{tip.location || 'Ortsunabhängig'}</span>
                         <span className="text-[8px] font-mono text-slate-600">{new Date(tip.timestamp).toLocaleDateString('de-DE')}</span>
                      </div>
                      <div className="text-[11px] font-bold text-white uppercase tracking-tight mb-4 truncate">{tip.title}</div>
                      <div className="flex gap-2">
                         <button onClick={() => navigate('/tips')} className="flex-1 bg-white/5 hover:bg-white/10 text-[9px] font-black uppercase tracking-widest py-2 rounded-lg transition-all text-slate-400">Details</button>
                         <button onClick={() => updateTipStatus(tip.id, 'Gelesen')} className="flex-1 bg-amber-600/20 hover:bg-amber-600 text-amber-400 hover:text-white text-[9px] font-black uppercase tracking-widest py-2 rounded-lg transition-all border border-amber-500/20">Gelesen</button>
                         <button onClick={() => updateTipStatus(tip.id, 'Archiviert')} className="h-8 w-8 flex items-center justify-center bg-black/40 hover:bg-red-600/20 text-slate-500 hover:text-red-500 rounded-lg border border-white/5 transition-all" title="Direkt Archivieren">📦</button>
                      </div>
                    </div>
                  ))}
                  {newTips.length === 0 && <div className="py-8 text-center opacity-20 text-[10px] font-black uppercase tracking-widest">Keine neuen Hinweise</div>}
                  {newTips.length > 4 && (
                    <button onClick={() => navigate('/tips')} className="w-full text-center text-[8px] font-black text-slate-500 hover:text-white uppercase tracking-widest mt-2 transition-colors">
                      Alle {newTips.length} Hinweise im Posteingang anzeigen ➔
                    </button>
                  )}
                </div>
             </div>
